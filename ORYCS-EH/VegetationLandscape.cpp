...
void VegetationLandscape::calculateProcesses(int year, int day, WaterLandscape* waterlandscape, Weather* we, Parameter* p){  /*Tong, based on PFT-oriented*/
    ...
    if (day == 0) {
        ...
        //fire_ini
        /// \todo fire routine needs to be reviewed! Fire parameters (e.g. which year?) should be a parameter read in from file
    ...
    ///----------------------------EVERY VEGTIMESTEP OUTSIDE WET SEASON----------------------------
    else if (day%vegTimeStep == vegTimeStep-1) {
    ...
    ///----------------------------END: EVERY VEGTIMESTEP OUTSIDE WET SEASON-------------------------------
    ...
    if (day == growEnd){
        ...
        for (int x = 0; x < xsize; x++) {
            for (int y = 0; y < ysize; y++) {
                ...
                veggrid[x][y]->gBasicMortRate_min = veggrid[x][y]->pftList[Shrubs]->BasicMortRate; /// \todo why only for first Perennial?
                for (int i = 0; i < PFTs; i++){
                    ...
                    if (i >= Shrubs && i < Perennials) {
                        if (veggrid[x][y]->pftList[i]->BasicMortRate < veggrid[x][y]->gBasicMortRate_min ){
                            veggrid[x][y]->gBasicMortRate_min = veggrid[x][y]->pftList[i]->BasicMortRate;  /// \todo why here only for last Perennial?
                        }
                    }
                    /// \implements \fn PlantFunctionalType::calculateBiomass(VegetationCell* vegetation, Weather *we2, int yr2)
                    veggrid[x][y]->pftList[i]->calculateBiomass(veggrid[x][y], we, year);
    ...
    ///----------------------------EVERY VEGTIMESTEP IF DAY != 335-------------------------------------------------
    else if ((day%vegTimeStep == vegTimeStep-1) && !((day - vegTimeStep < growEnd) && (day > growEnd))){ /// \todo Don't understand the second if-argument. Will not be done, when day == 335, but why?
        for (int x = 0; x < xsize; x++) {
            for (int y = 0; y < ysize; y++) {
                for (int i = 0; i < PFTs; i++){
	...
	///----------------------------END: EVERY VEGTIMESTEP IF DAY != 335--------------------------------------------

    ///----------------------------AFTER WET SEASON BUT NOT LAST DAY-----------------------------------------------
    if((day >= wetEnd) && (day < 364)){
        for (int x = 0; x < xsize; x++){
            for (int y = 0; y < ysize; y++){
    ...
    ///----------------------------END: AFTER WET SEASON BUT NOT LAST DAY------------------------------------------
    ...
    if (day == 364) {
        ...
        //-------------------------------------
        oldstock = realstock; /// \todo why new variable and not just hand over realstock?
    ...
    for (int x = 0; x < xsize; x++){
        for (int y=0; y<ysize; y++) {
            ...
            for (int i = Shrubs; i < Shrubs + Perennials; i++){
                ...
                if (zuf < 0.01){
                    // there has a problem, for multi-perennials and one perennial, it has a difference, one perennial has one possibility of increasing 0.02,
                    //  multi-perennials has several possibilities of increasing 0.02
                    veggrid[x][y]->pftList[i]->Disp += 0.02/Perennials; /// \todo Btodo: why is this random process added this way, shouldn't it be neutral, i.e. between -0.01 and +0.01?
                }
            }
        }
    }
        ...
        for (int y = 0; y < ysize; y++) {
            ...
            if (coveradded > coverleft){
                ...
                for (int i = 0; i < Shrubs; i++){
                    ...
                    if (zuf < 0.001){
                        // there has a problem, for multi-shrubs and one shrub, it has a difference, one shrub has one possibility of increasing 0.02,
...
void VegetationLandscape::meanCoverBiomass(){/*Tong, based on PFT-oriented*/
    ...
    for (int i = 0; i < 10; i++){
        ...
        /// \todo what is sCsumabove? sCsumabove was not correctly initilised, only Shrubs but here until 10
        sCsumabove[i] = 0;
    }
    ...
    for (int i = 0; i < xsize; i++) {
        for (int j = 0; j < ysize;j++) {
            for (int m = 0; m < PFTs; m++){
                ...
                if (fraction[m] < 0) cout << "Grazing fraction zu klein" << "\t" << veggrid[i][j]->pftList[m]->Biomassold << "\t" << veggrid[i][j]->pftList[m]->Biomass<< "\n";
...
/*******************************************************************************************
 * calculate biodiversity indices (Richness, ...) including total PFTs present in the landscape
 * will add one to Richness if 0.5% cover are found in at least 1 cell
...
void VegetationLandscape::calculateMean(int day) {/*Tong, based on PFT-oriented*/
    ...
    if (meanRCover < 0.01) {
        meanRCover = 0;
    } else meanRCover = meanRCover / (1.0*xsize*ysize);

    for (int i = 0; i < PFTs; i++){
    	meanCover[i] = meanCover[i]/(1.0*xsize*ysize);
    	meanML1[i] = meanML1[i]/(1.0*xsize*ysize);
    	meanML2[i] = meanML2[i]/(1.0*xsize*ysize);
    	meandML1[i] = meandML1[i]/(1.0*xsize*ysize);
    	meandML2[i] = meandML2[i]/(1.0*xsize*ysize);
    	meanGrowthL1[i] = meanGrowthL1[i]/(1.0*xsize*ysize);
    	meanGrowthL2[i] = meanGrowthL2[i]/(1.0*xsize*ysize);
    	meanBasicMort[i] = meanBasicMort[i]/(1.0*xsize*ysize);
    ...
    for (int i = 0;i < Shrubs;i++){
        SDSC[i] = sqrt(sumshrub[i]/(1.0*xsize*ysize-1.0));
    }

    if (day == growEnd) {
        for (int i = 0; i < PFTs; i++){
            meanDryMort[i] = meanDryMort[i]/(1.0*xsize*ysize);
            meanDryMortL1[i] = meanDryMortL1[i]/(1.0*xsize*ysize);
            meanDryMortL2[i] = meanDryMortL2[i]/(1.0*xsize*ysize);
            meanDisp[i] = meanDisp[i]/(1.0*xsize*ysize);
            meanDispSend[i] = meanDispSend[i]/(1.0*xsize*ysize);
            sumGrowth[i] = sumGrowth[i]/(1.0*xsize*ysize); //_dirk summiert Wachstum uebers ganze Jahr und mittelt auf die Zelle
        }
	}

	meanMoistSeasonL1 = meanMoistSeasonL1/(1.0*xsize*ysize);
	meanMoistSeasonL2 = meanMoistSeasonL2/(1.0*xsize*ysize);
	meanMoistWetSeasonL1 = meanMoistWetSeasonL1/(1.0*xsize*ysize);
	meanMoistWetSeasonL2 = meanMoistWetSeasonL2/(1.0*xsize*ysize);

    for (int i = 0; i < PFTs; i++){
        meanT1[i] = meanT1[i]/(1.0*xsize*ysize);
        meanT2[i] = meanT2[i]/(1.0*xsize*ysize);
    }

    meanTL1 = meanTL1 / (1.0*xsize*ysize);
    meanTL2 = meanTL2 / (1.0*xsize*ysize);
    meanE = meanE / (1.0*xsize*ysize);
...
void VegetationLandscape::initialiseResultFile(Parameter* p){/*Tong, based on PFT-oriented*/
    ...
    yearlySummaryFileName = "Results" + delimiter + p->site + "_" + p->convertIntToString(p->simYears) + "years_yearly_" +
