...
/*******************************************************************************************
 * calculation of some temperature dependent general parameters
 * if temperature data is availiable for the whole day
 * is called in --> WaterLandscape::potEvapotranspiration
 *******************************************************************************************/
void Weather::generalParameters(int year, int day) {

	daynumber = year*daysPerYear + day;

	if (tempAvg[daynumber]!=nodata){
		satVP = 0.6108*exp(17.27*tempAvg[daynumber]/(273.3+tempAvg[day]));  //[kPa]
		gradVP = 4098*satVP/pow(273.3+tempAvg[daynumber],2);	            //[kPa/^C]
		latHeat_J = 2.501-0.002361*tempAvg[daynumber];  		            //[mJ/kg]
		latHeat_W = 28.9-0.028*tempAvg[daynumber];			                //[(W/m²)/(mm/d)]
		psyConst = 0.0016286*atmP_kPa/latHeat_J;		                    //[kPa/°C]
	}
}
...
void Weather::getWeather(Parameter* p){
    ...
    parameterFileName = "Weather" + delimiter + p->site + "_" + p->convertIntToString(p->simYears) + "years_" + p->weatherFileID[p->scenario] + "_climrep-" + p->convertIntToString(p->climateRepetition + 1) + ".txt";
