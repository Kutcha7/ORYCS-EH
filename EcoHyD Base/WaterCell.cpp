...
void WaterCell::actLayerEvapotranspiration(double rainTot, double potEP){ /*Tong, based on PFT-oriented*/
	...
	if (potEP != nodata){
		...
		double potL0EP = potEP * (1 - (cStotal + cAtotal + cGtotal) / 2.0) * cos(inclination);
		...
		if (waterL1_rel > WP){
			...
			EPL1 = min(potEP * cos(inclination) * vegfunctionL1 * EPfactor,(waterL1_rel - resWater) * depthL1);
		}
		else
			EPL1 = max(0.0,min(potEP * pow((double)waterL1_rel / WP,2) * cos(inclination * vegfunctionL1) * EPfactor,(waterL1_rel - resWater) * depthL1));

		///--------------------------transpiration in lower soil layer----------------------------------------------------
        if (waterL2_rel <= WP)
			EPL2 = max(0.0,min(potEP * pow((double)waterL2_rel / WP,2) * vegfunctionL2 * cos(inclination),(waterL2_rel - resWater) * depthL2));

		else
	        EPL2 = min(potEP * cos(inclination) * vegfunctionL2,(waterL2_rel - resWater) * depthL2);
