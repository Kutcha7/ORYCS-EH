...
void VegetationCell::calculateWaterUptake() {  /*Tong, based on PFT-oriented*/
	...
	for (int i = 0; i < PFTs; i++){
		pftList[i]->URL1_scaled = pftList[i]->UptakeRateCover * pftList[i]->RootL1 * pftList[i]->Cover;
		pftList[i]->URL1_help = pftList[i]->UptakeRateCover * pftList[i]->RootL1;
        pftList[i]->URL2_scaled = pftList[i]->UptakeRateCover * pftList[i]->RootL2 * pftList[i]->Cover;
...
void VegetationCell::calculatePFTfire(){
    ...
    int fireIntens = 2729 + 0.8684 * fuel2 - (530*sqrt(FM)) - (0.1907*relhum*relhum) - (596/windspeed);   //fire intensity is:  fuel in kg/ha
